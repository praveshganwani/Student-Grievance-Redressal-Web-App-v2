...
@WebServlet("/ForwardsController")
public class ForwardsController extends HttpServlet{
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(ac != null) {
            ...
            for (Iterator<Activity> i = listOfActivities.iterator(); i.hasNext();) {
                ...
                if(activity.getActivityType().equals("forward")) {
                    flag = 1;
                    break;
                }
            }
            if(flag == 0) {
                ...
                a.setActivityTo(req.getParameter("forwardTo"));
                a.setActivityComment(req.getParameter("forwardComments"));
                a.setActivityType("forward");
                ...
                webTarget = client.target(DBConfig.getApiHost()).path("additional/"+a.getActivityTo());
                invocationBuilder =  webTarget.request(MediaType.APPLICATION_JSON);
                response = invocationBuilder.get();
                NameById nbi = response.readEntity(NameById.class);
                Email e = new Email();
                e.setActivityFrom(ac.getCommitteeDetails().getCommitteeName());
                e.setG(gr);
                e.setSubject("Complaint Forwarded");
                e.setTitle("Your Complaint Was Forwarded to "+nbi.getName());
